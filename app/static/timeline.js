...
function tlTotalLength() {
  return tl.clips.reduce((sum, clip) => sum + tlClipLength(clip), 0);
}
...
function tlAddTitleCard() {
  tl.clips.push({
    ...
    text: t("tlNewTitleText"), subtitle: null,
  });
  tl.selected = tl.clips.length - 1;
  tlMarkDirty();
  tlRenderStrip();
...
function tlRenderStrip() {
  ...
  tl.clips.forEach((clip, index) => {
    if (index > 0) {
      ...
      join.type = "button";
      join.className = `tl-join ${clip.transition === "fade" ? "fade" : "cut"}`;
      join.title = t(clip.transition === "fade" ? "tlJoinFade" : "tlJoinCut");
      join.textContent = clip.transition === "fade" ? "◑" : "▮";
      ...
      join.addEventListener("click", () => {
        clip.transition = clip.transition === "fade" ? "cut" : "fade";
        tlMarkDirty();
        tlRenderStrip();
        tlRenderInspector();
      });
...
function tlRenderInspector() {
  ...
  panel.appendChild(tlField(t("tlTransition"), tlSelect(
    [["cut", t("tlTransitionCut")], ["fade", t("tlTransitionFade")]],
    clip.transition || "cut",
  ...
  if (clip.transition === "fade") {
    panel.appendChild(tlField(t("tlFadeLength"), tlNumber(
      ...
      (value) => { clip.fade_duration = value; tlMarkDirty(); },
    )));
  }
...
function tlRenderAudio() {
  ...
  tl.audioTracks.forEach((track, index) => {
    ...
    preview.src = track.signed_url || "";
    if (track.signed_url) row.appendChild(preview);
...
function tlAddAudioTrack(url, label, signedUrl) {
  ...
  tl.audioTracks.push({
    url, volume: 0.25, label: label || t("tlAudioTrack"), signed_url: signedUrl || null,
  });
  tlMarkDirty();
  tlRenderAudio();
}
...
function tlPayload() {
  return {
    ...
    audio_tracks: tl.audioTracks.map(({ url, volume, label }) => ({ url, volume, label })),
  };
}
...
function tlReset() {
  clearTimeout(tl.pollTimer);
  Object.assign(tl, {
    id: null, name: "", aspectRatio: "16:9", clips: [], audioTracks: [],
